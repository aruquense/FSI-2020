...
def graph_search(problem, fringe):
    ...
    while fringe:
        node = fringe.pop()
        #print( node.path()," accumulated path cost --> ", node.path_cost)
        if node.state in closed:
            print("                  ", node, "       Ya se expandió y está en la lista cerrada")

        if problem.goal_test(node.state):
            print("Numero de nodos expandidos --> ", counter)
            print("Numero de nodos visitados  --> ", counter+1)
            return node
        if node.state not in closed:
            closed[node.state] = True
            fringe.extend(node.expand(problem))
            #CONTAR AQUI LOS NODOS EXPANDIDOS
            counter=counter+1
            print("Expandido nodo -->",node, "  ||   Altura : ",node.depth)
    return None
...
def ramificacionyacotacion(problem):
    return graph_search(problem, FIFOQueueSorted())


def ramificacionyacotacionconsubestimacion(problem):
    return graph_search(problem, FIFOQueueSortedwithHeuristic(problem))