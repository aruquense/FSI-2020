...
class FIFOQueueSorted(Queue):
    ...
    def extend(self, items):
        self.A.extend(items)
        self.A.sort(key=lambda self: self.path_cost,reverse=True)
...
class FIFOQueueSortedwithHeuristic(Queue):
    ...
    def extend(self, items):
        self.A.extend(items)
        self.A.sort(key=lambda t: t.path_cost+self.problem.h(t),reverse=True)
